...
import { formatEur, formatDate, whatsappUrl } from '../lib/constants'
...
function EyeIcon() { return <svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M1 6.5S3 2 6.5 2 12 6.5 12 6.5 10 11 6.5 11 1 6.5 1 6.5z"/><circle cx="6.5" cy="6.5" r="1.5"/></svg> }
...
function FormularioPropuesta({ propuesta, clientes, onSave, onCancel }) {
  ...
  const [form, setForm] = useState({
    cliente_id: '',
    titulo: 'Propuesta de servicios de automatización e IA',
...
Abre el enlace para ver la propuesta completa. Cualquier duda estoy disponible! 🚀
...
Propuesta válida durante ${propuesta.validez || 30} días.

Para aceptar la propuesta o resolver cualquier duda, no dudes en contactarme.

Un saludo,
${respNombre}
ONESEVEN IA
${respEmail} · onesevenia.com`

  const abrirPDF = () => {
    const win = window.open('', '_blank')
    win.document.write(generarHTMLPropuesta({ p: propuesta, cliente }))
    win.document.close()
  ...
  const enviarWA = () => {
    ...
    const url = `https://wa.me/${phone}?text=${encodeURIComponent(msgWA)}`
    setTimeout(() => { window.open(url, '_blank'); onEnviado('wa') }, 1200)
  }

  const enviarEmail = () => {
    abrirPDF()
    const url = `mailto:${cliente?.email || ''}?subject=${encodeURIComponent(asuntoEmail)}&body=${encodeURIComponent(msgEmail)}`
    setTimeout(() => { window.open(url); onEnviado('email') }, 1200)
  }
      ...
      <div style={{ background: 'var(--bg1)', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: 560, border: '1px solid var(--border)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '16px 20px 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--bg3)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', marginBottom: 14 }}>
            <DocIcon />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text0)' }}>La propuesta se abrirá como PDF</div>
              <div style={{ fontSize: 11, color: 'var(--text3)' }}>Guárdala en tu dispositivo y adjúntala al mensaje</div>
            </div>
            <button className="btn btn-ghost btn-sm" onClick={abrirPDF}><EyeIcon /> Ver PDF</button>
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '12px', fontSize: 12, color: 'var(--text1)', lineHeight: 1.7, whiteSpace: 'pre-wrap', maxHeight: 200, overflowY: 'auto', fontFamily: tab === 'wa' ? 'inherit' : 'var(--mono)', fontSize: 11 }}>
          ...
          {/* Botón envío */}
          <button
            onClick={tab === 'wa' ? enviarWA : enviarEmail}
            style={{ width: '100%', padding: '13px', borderRadius: 'var(--radius)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 14, fontWeight: 600, color: '#fff', background: tab === 'wa' ? '#25d366' : 'var(--accent)', transition: 'opacity 0.15s' }}
          >
            {tab === 'wa' ? <WAIcon /> : <MailIcon />}
            {tab === 'wa' ? `Abrir PDF + WhatsApp a ${cliente?.nombre?.split(' ')[0]}` : `Abrir PDF + Email a ${cliente?.email}`}
          </button>
          <div style={{ fontSize: 11, color: 'var(--text3)', textAlign: 'center', marginTop: 8 }}>
            Se abrirá el PDF para que puedas guardarlo, luego se abrirá {tab === 'wa' ? 'WhatsApp' : 'tu cliente de correo'} con el mensaje listo
          </div>
        </div>
      </div>
    </div>
...
export default function Propuestas() {
  ...
  const [vista, setVista] = useState('lista') // lista | form | preview
  ...
  const totalPropuestas = propuestasFiltradas.reduce((s, p) => s + ((p.items || []).reduce((ss, it) => ss + (parseFloat(it.precio) || 0), 0)), 0)
      ...
      {propuestasFiltradas.length === 0 && (
        <div className="card empty-state">
          <div className="empty-state-text">
            {busqueda || filtroEstado ? 'No hay propuestas con estos filtros' : 'Crea tu primera propuesta'}
          </div>
        </div>
      )}
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, fontSize: 11, color: 'var(--text3)', flexWrap: 'wrap' }}>
                    ...
                    {p.fecha_envio && <span>Enviada {formatDate(p.fecha_envio)}</span>}
                  </div>
                  {/* Items resumen */}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  {total > 0 && (
                    <span style={{ fontSize: 15, fontWeight: 700, fontFamily: 'var(--mono)', color: 'var(--green)' }}>
                      {formatEur(total)}
                    </span>
                  )}
                  ...
                  {cliente?.telefono && (
                    <button
                      className="btn-icon"
                      style={{ width: 32, height: 32, color: '#25d366', borderRadius: 8, border: '1px solid rgba(37,211,102,0.3)', background: 'rgba(37,211,102,0.08)' }}
                  ...
                  {cliente?.email && (
                    <button
                      className="btn-icon"
                      style={{ width: 32, height: 32, color: 'var(--accent2)', borderRadius: 8, border: '1px solid var(--accent-dim)', background: 'var(--accent-dim)' }}
                      onClick={() => setEnviando(p)}
                  ...
                  <button
                    className="btn-icon"
                    style={{ width: 32, height: 32, color: 'var(--text2)' }}
                    onClick={() => {
                      const win = window.open('', '_blank')
                      win.document.write(generarHTMLPropuesta({ p, cliente }))
                      win.document.close()
                      setTimeout(() => win.print(), 600)
                    }}
                    title="Ver PDF"
                  ...
                  <button
                    className="btn-icon"
                    style={{ width: 32, height: 32, color: 'var(--text2)' }}
                    onClick={() => { setEditando(p); setVista('form') }}
                    title="Editar"
                  >
                    <EditIcon />
                  </button>
