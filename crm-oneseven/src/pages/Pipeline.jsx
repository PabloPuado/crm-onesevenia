...
function TrashIcon() { return <svg width='12' height='12' viewBox='0 0 13 13' fill='none' stroke='currentColor' strokeWidth='1.5'><path d='M2 3h9M5 3V2h3v1M3 3l.5 8h6l.5-8'/></svg> }
function PlusIcon() {
    ...
    >
                    ...
                    {c.proximo_paso && (
                      <div style={{ fontSize: 11, color: 'var(--amber)', background: 'rgba(245,158,11,0.1)', borderRadius: 6, padding: '4px 7px', marginBottom: 8, display: 'flex', alignItems: 'flex-start', gap: 5 }}>
                        ...
                        <span style={{ lineHeight: 1.4 }}>
                          {c.proximo_paso}
                          {c.fecha_proximo_paso && <span style={{ opacity: 0.7, marginLeft: 4 }}>· {new Date(c.fecha_proximo_paso).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</span>}
                        </span>
                      </div>
                    )}
                    <div className="kanban-card-footer">
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <button
                          ...
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--red)', padding: 2, opacity: 0.6, display: 'flex', alignItems: 'center' }}
                          title="Eliminar"
                        ><TrashIcon /></button>
                        <div
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={e => e.target === e.currentTarget && setConfirmDelete(null)}>
          <div style={{ background: 'var(--bg1)', borderRadius: 'var(--radius-lg)', padding: '24px', maxWidth: 380, width: '100%', border: '1px solid var(--border)', textAlign: 'center' }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text0)', marginBottom: 8 }}>Eliminar a {confirmDelete?.nombre}?</div>
            <div style={{ fontSize: 12, color: 'var(--red)', marginBottom: 20, padding: '8px 12px', background: 'rgba(239,68,68,0.1)', borderRadius: 8 }}>
              Se eliminara de todos los sitios donde este asociado.
            </div>
